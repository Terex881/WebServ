...
int	check_port(vector<string> line)
{
	...
	if (line[1].length() != (index + 1))
		return 0;
	if (!(line[1].length() >= 2) || !(line[1].length() <= 5))
		return (0);
	line[1] = line[1].substr(0, index);
	num = atoi(line[1].c_str());
	if (num == 0)
		return 0;
...
void File_Parsing::Checking_Hierarchy(const DynamicStruct &block, DynamicStruct *server, DynamicStruct *locations, const string &name)
{
	...
	if (name == "servers" && (block.values.size() < 3 || block.children.size() != 1))
		exit(1);
	...
	for (map<string, vector<DynamicStruct> >::const_iterator it = block.children.begin(); it != block.children.end(); ++it)
	{
		const string &block_name = it->first;
		const vector<DynamicStruct> &blocks = it->second;

		for (vector<DynamicStruct>::const_iterator child_it = blocks.begin(); child_it != blocks.end(); ++child_it)
		{
