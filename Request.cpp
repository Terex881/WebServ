...
void parseFirstLine(string line, map<string, string> &mp)
{
	...
	if (it->second != "POST" && it->second != "GET" && it->second != "DELETE")
		cout << "error: 501 Not Implemented\n";
	it = mp.find("path");
...
void parseChunkedBody(string line, std::ofstream &tmp)
{
    ...
    while (1337)
	{
		...
		if (l == 0){
			if (!line.empty())
				cout << "error: strtol doesn't found an hexadecimal return 0"; // if the length not valid strtol fails return \n0
			break;
		}
		trim(line);
		res = line.substr(0, l);
		if ((int)l != (int)res.length())
			cout << "error: the length of the chunked data dosn't match\n";
		tmp << res;
		line.erase(0, l);
		if (line[0] != '\r' && line[1] != '\n')
			cout << "error: found \r\n inside line"; // ckech if it fails to close the \nfile
		trim(line);
    }
}
...
void parseBodyTypes(string line, map<string, string> mp)
{
	...
	else if (mp.find("Content-Length") != mp.end())
	{
		...
		cout << "line is:" << it->second.c_str() << endl;
		cout << "line is:" << line.length()  << endl;
