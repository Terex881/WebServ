...
/*   Updated: 2025/01/04 11:01:33 by sdemnati         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Body::parseBodyTypes(string &body)
{
	...
	string last = newStr.substr(std::max(0, (int)(newStr.length() - atay_tkhwa->data.endBoundry.length())));

	if (hasOneMatch(last, atay_tkhwa->data.endBoundry) && newStr.find(atay_tkhwa->data.endBoundry) == string::npos)
		return;
			
	switch (atay_tkhwa->data.bodyType)
	{
